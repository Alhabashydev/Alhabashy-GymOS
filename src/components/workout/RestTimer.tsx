  ...
  return (
    <div className="fixed inset-x-4 bottom-24 z-40 mx-auto max-w-md rounded-card border border-white/10 bg-surface/95 p-4 shadow-glowStrong backdrop-blur-xl md:bottom-6">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-muted">{t('timer.title')}</p>
          <p className="font-display text-2xl font-bold text-text">{remaining === 0 ? t('timer.ready') : `${mins}:${secs}`}</p>
        </div>
        <IconButton label={t('timer.close')} onClick={onClose}><X size={18} /></IconButton>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Button variant="secondary" onClick={() => setRemaining(seconds)} leftIcon={<TimerReset size={16} />}>{t('timer.restart')}</Button>
        <Button onClick={onClose}>{t('timer.skip')}</Button>
      </div>
    </div>
  );
}
